...
export default function ServicesSection({ onOpenCart }: ServicesSectionProps) {
  ...
  const [selectedService, setSelectedService] =
    useState<ServiceCardItem | null>(null);
  ...
  const [
    paidFileDownloadPendingServiceId,
    setPaidFileDownloadPendingServiceId
  ...
  const arrowButtonClass =
    'y2k-button y2k-button-ghost y2k-button-icon size-11';
  ...
  const adminCloseButtonClass = 'y2k-button y2k-button-ghost y2k-button-icon';
  ...
  const mapPostToCardItem = (
    ...
    return {
      ...
      filePriceAmount: Number.isFinite(parsedFilePrice ?? NaN)
        ? parsedFilePrice
        : null,
      downloadFileObjectKey:
        typeof post.download_file_url === 'string' &&
  ...
  const toCartItem = (service: ServiceCardItem): CartItemInput => ({
    ...
    title: service.title,
    image: service.image,
    price: service.isPaidFile
      ? (service.filePriceAmount ?? service.priceAmount)
      : service.priceAmount,
  ...
  const handleAddToCart = (service: ServiceCardItem) => {
    if (service.isPaidFile && !user) {
      toast({
        title: '로그인이 필요합니다',
        description:
          '유료 3D 파일은 로그인 후 결제해야 다운로드 권한을 저장할 수 있습니다.'
      });
      return;
    }
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || '서비스 목록을 불러오지 못했습니다.'
        );
      }

      const rows = Array.isArray(payload?.data)
        ? (payload.data as ServicePost[])
        : [];
      setServiceItems(
        rows.length > 0 ? rows.map((row) => mapPostToCardItem(row)) : []
      );
    } catch (error) {
      setServicesError(
  ...
  const handleCreateFormFieldChange = (
    ...
    setCreateForm(
      (prev) => ({ ...prev, [key]: value }) as ServiceCreateFormState
    );
  };

  const handleCreateFilesChange = (
  ...
  const handlePaidDownloadFileChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const appendServiceContentImageUrls = (
    current: string,
    imageUrls: string[]
  ) => {
  ...
  const uploadServiceContentImages = async (files: File[]) => {
    ...
    if (!uploadResponse.ok) {
      throw new Error(
        uploadPayload?.message || '상세 내용 이미지 업로드에 실패했습니다.'
      );
    }

    const uploadedImageUrls = Array.isArray(uploadPayload?.data?.image_urls)
      ? (uploadPayload.data.image_urls as string[])
  ...
  const handleCreateContentDrop = async (
  ...
  const uploadServicePaidFileToR2 = async (
    servicePostId: string,
    file: File
  ) => {
    const contentType =
      (file.type || '').trim().toLowerCase() || 'application/octet-stream';
    ...
    if (
      ...
      throw new Error(
        presignPayload?.message || '3D 파일 업로드 URL 발급에 실패했습니다.'
      );
    }
  ...
  const handleSubmitCreatePost = async () => {
    ...
    if (createContentUploading) {
      setCreateError(
        '상세 내용 이미지 업로드가 끝난 뒤 게시글을 생성해 주세요.'
      );
      setCreateMessage(null);
      return;
    }
    ...
    try {
      ...
      if (createForm.files.length > 0) {
        ...
        if (!uploadResponse.ok) {
          throw new Error(
            uploadPayload?.message || '이미지 업로드에 실패했습니다.'
          );
        }
      ...
      const response = await fetch('/api/service-posts', {
        ...
        body: JSON.stringify({
          ...
          price_from: createForm.price_from
            ? Number(createForm.price_from)
            : null,
          currency: createForm.currency || 'KRW',
          is_paid_file: createForm.is_paid_file,
          file_price: createForm.is_paid_file
            ? Number(createForm.file_price)
            : null,
      ...
      if (
        ...
        const patchResponse = await fetch(
          `/api/service-posts/${createdServicePostId}`,
          {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (!patchResponse.ok) {
          throw new Error(
            patchPayload?.message ||
              '게시글은 생성되었지만 다운로드 파일 연결에 실패했습니다.'
          );
        }
      }
  ...
  const handleCheckoutPaidFile = (service: ServiceCardItem) => {
    if (!user) {
      toast({
        title: '로그인이 필요합니다',
        description:
          '유료 3D 파일 구매 후 다운로드를 위해 로그인한 상태에서 결제해 주세요.'
      });
      return;
    }
  ...
  const handleDownloadPaidFile = async (service: ServiceCardItem) => {
    ...
    try {
      const response = await fetch(
        `/api/service-posts/${service.id}/download`,
        {
          cache: 'no-store'
        }
      );
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || '다운로드 링크를 발급하지 못했습니다.'
        );
      }
      const url =
  ...
  const openServiceDetail = async (service: ServiceCardItem) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message || '서비스 상세를 불러오지 못했습니다.'
        );
      }
      const detail = payload?.data as ServicePost | undefined;
      const viewerHasPaidFileAccess = Boolean(
        payload?.meta?.viewer_has_paid_file_access
      );
      if (detail?.id) {
        setSelectedService(
  ...
  const renderServiceSkeletonCards = (count: number, mobile = false) =>
    Array.from({ length: count }).map((_, index) => (
      ...
      >
        ...
        <div
          className={`${mobile ? 'hidden' : 'flex'} justify-center gap-2 py-4`}
        >
        ...
        <div
          className={`${mobile ? 'space-y-2 px-0 pb-0 pt-3' : 'space-y-3 p-6'}`}
        >
          <div
            className={`${mobile ? 'h-4 w-full' : 'h-6 w-3/4'} rounded bg-white/10`}
          />
          <div
            className={`${mobile ? 'h-3 w-2/3' : 'h-4 w-1/2'} rounded bg-white/10`}
          />
          {!mobile ? (
            <>
          ...
          <div
            className={`${mobile ? 'mt-1 h-4 w-3/5' : 'mt-2 h-5 w-24'} rounded bg-white/10`}
          />
          <div
            className={`flex ${mobile ? 'flex-col gap-1.5' : 'gap-2.5 pt-1'}`}
          >
    ...
    >
        ...
        >
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-cyan-50/55">
                  Goods
                </p>
              ...
              <button
                ...
                onClick={() =>
                  !(createSubmitting || createContentUploading) &&
                  setIsCreateModalOpen(false)
                }
                aria-label="작성 모달 닫기"
              >
            ...
            <div className="grid gap-4">
              <div className="grid gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleCreateFormFieldChange('title', e.target.value)
                  }
                  placeholder="서비스 제목"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="grid gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleCreateFormFieldChange('category', e.target.value)
                    }
                    disabled={createSubmitting}
                  >
                ...
                <div className="grid gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleCreateFormFieldChange('price_from', e.target.value)
                    }
                    placeholder="150000"
                    disabled={createSubmitting}
                  />
              ...
              <div className="grid gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleCreateFormFieldChange('summary', e.target.value)
                  }
              ...
              <div className="grid gap-2">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    handleCreateFormFieldChange('content', e.target.value)
                  }
                  onDragOver={(event) => {
                    event.preventDefault();
              ...
              <div className="grid gap-2">
                ...
                {createForm.files.length > 0 && (
                  <p className="text-xs text-cyan-50/55">
                    선택됨:{' '}
                    {createForm.files.map((file) => file.name).join(', ')}
                  </p>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-cyan-100/20 bg-cyan-200/10 p-4">
                ...
                <p className="mt-2 text-xs text-cyan-50/55">
                  체크 시 결제 완료 사용자에게만 3D 파일 다운로드 버튼이
                  노출됩니다.
                </p>
                  ...
                  <div className="mt-4 grid gap-4">
                    <div className="grid gap-2 md:max-w-sm">
                      ...
                      <input
                        ...
                        placeholder={
                          createForm.currency === 'USD' ? '19.99' : '4900'
                        }
                        disabled={createSubmitting}
                      />
                    </div>
      ...
      <ServiceDetailModal
        ...
        paidFileDownloadPending={
          ...
          paidFileDownloadPendingServiceId === selectedService?.id
        }
