...
interface ServiceDetailModalProps {
  ...
  error?: string | null;
  onAddToCart?: (
    service: NonNullable<ServiceDetailModalProps['service']>
  ) => void;
  onPaidFileCheckout?: (
    service: NonNullable<ServiceDetailModalProps['service']>
  ) => void;
...
export function ServiceDetailModal({
  ...
  const paidFilePriceText = service?.isPaidFile
    ? (formatMoneyExact?.(service.filePriceAmount, service.currency) ??
      service?.price ??
      null)
    : null;
  ...
  if (game)
    return (
      ...
      >
        <Dialog.Portal>
          <Dialog.Overlay className={adventure.inspectionOverlay} />
          <Dialog.Content
          ...
          >
            <header className={adventure.roomHeader}>
              <Dialog.Title>단의 물건 이야기</Dialog.Title>
              <Dialog.Close className={adventure.exit}>
                <ArrowLeft size={18} />
                진열대로
              </Dialog.Close>
            </header>
            <div className={adventure.activityContent}>
              <div className={adventure.exhibit}>
                {isLoading ? (
                  <p role="status">물건의 기록을 펼치는 중...</p>
                ) : error ? (
                  <p className={adventure.error} role="alert">
                    {error}
                  </p>
                ) : (
                  service && (
                    <>
                      <div className={adventure.objectStage}>
                        <button
                      ...
                      <div className={adventure.objectInfo}>
                        <div>
                        ...
                        <button
                          ...
                          disabled={
                            ...
                            (paidFileDownloadPending ||
                              !service.downloadFileObjectKey)
                          }
                          onClick={() =>
                            service.isPaidFile
                              ? service.hasPurchasedPaidFile
                                ? onPaidFileDownload?.(service)
                                : onPaidFileCheckout?.(service)
                              : onAddToCart?.(service)
                          }
                        >
                          {service.isPaidFile ? (
                          ...
                          {service.isPaidFile
                            ? service.hasPurchasedPaidFile
                              ? paidFileDownloadPending
                                ? '다운로드 준비 중...'
                                : '구매한 파일 다운로드'
                              : `파일 구매 (${paidFilePriceText || service.price})`
                            : '장바구니에 담기'}
                        </button>
                      </div>
          ...
          {!isLoading && !error && service && (
            <>
              ...
              <div className="p-4 sm:p-5 md:p-8">
                ...
                {/* Title */}
                <h2 className="mb-2 break-words text-2xl leading-none tracking-tight text-white sm:text-3xl md:text-4xl">
                  {service.title}
                </h2>

                {/* Price */}
                <p className="mb-4 break-words text-lg text-white sm:text-xl">
                  {service.price}
                </p>
