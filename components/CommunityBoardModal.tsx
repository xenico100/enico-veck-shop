...
type CommunityBoardModalProps = {
  onOpenChange: (open: boolean) => void;
  open: boolean;
};

export default function CommunityBoardModal({
  onOpenChange,
  open
}: CommunityBoardModalProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="fixed inset-0 z-[88]">
      <button
        type="button"
        aria-label="커뮤니티 보드 닫기"
        className="absolute inset-0 bg-[rgba(10,8,14,0.62)] backdrop-blur-[4px]"
        onClick={() => onOpenChange(false)}
      />

      <div className="absolute inset-x-2 bottom-2 top-20 z-[89] overflow-hidden rounded-[1.55rem] border border-[rgba(183,61,61,0.18)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(250,246,250,0.94))] shadow-[0_40px_120px_rgba(34,14,14,0.24)] md:inset-x-6 md:bottom-6 md:top-24 md:rounded-[2rem]">
