...
export async function createStudioPost(
  ...
  const filePath = `public/posts/${user.id}/${postId}/${fileName}`;
...
export async function updateStudioPost(
  ...
  if (file instanceof File && file.size > 0) {
    const { data: postOwner } = await (supabase as never)
      .from('studio_posts')
      .select('user_id')
      .eq('id', postId)
      .maybeSingle();
    const ownerId = postOwner?.user_id ?? user.id;
    ...
    const fileExt = file.name.split('.').pop() ?? 'png';
    const fileName = `${randomUUID()}.${fileExt}`;
    const filePath = `public/posts/${ownerId}/${postId}/${fileName}`;
    const buffer = Buffer.from(await file.arrayBuffer());
